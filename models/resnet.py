...
class ResNet(nn.Module):
    ...
    def forward_in_exp(self, x, rep=20, sigma=1.0, noise='normal'):
        batch_size = x.size()[0]
        x = x.repeat(rep, 1, 1, 1)
        if noise == 'normal':
        ...
        x = self.feats(x + noise)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        # tmp = torch.zeros([batch_size, x.size()[-1]], requires_grad=True).cuda()

        for i in range(batch_size):
            for j in range(1, rep):
                x[i].data.add_(x[j * batch_size + i])

        return x[:batch_size] / rep
